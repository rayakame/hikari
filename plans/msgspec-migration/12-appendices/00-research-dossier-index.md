...
| 08 | `08-events-interactions.md` | Event/interaction structure, construction, app handling | Events are hand-constructed, never JSON-decoded; 44 own-`app` event fields (recount: **45** — the dossier's `attrs.field` grep missed the `attr`-alias declaration in `auto_mod_events.py`) + 32 `app`-delegating properties that BREAK when the wrapped entity loses `.app`; interactions are entities-that-are-also-clients (subclass `ExecutableWebhook`); response builders need no `app`; 5 loose `Enum\|int` unions | [07-events/00-events-migration.md](../07-events/00-events-migration.md), [03-app-removal-and-helpers/04-events-and-interactions-app-decision.md](../03-app-removal-and-helpers/04-events-and-interactions-app-decision.md), [03-app-removal-and-helpers/02-helper-method-inventory/06-interactions.md](../03-app-removal-and-helpers/02-helper-method-inventory/06-interactions.md); D10-events (RESOLVED — superseded in part by 17–19), D10-interactions (RESOLVED — the maintainer chose app-less; the dossier's builder-factory verification carries into the kept, app-free factories) |
...
| 10 | `10-rest-client-surface.md` | REST public methods, deserialize/serialize call sites, DX shift | ~188 async endpoints; `impl/rest.py` has **zero** `.app` refs (REST is structurally safe); 90 helpers wrap **existing** endpoints (no mandatory new endpoint); only `User.send` is genuinely compound; `event.app.rest.*` was the dossier's replacement-path suggestion — since superseded by D10-events AND D10-interactions (events and interactions are both app-less; handlers close over the bot object and interaction callers use `rest.*` directly, every deleted helper wrapping an existing endpoint) | [09-rest-and-gateway/00-rest-client.md](../09-rest-and-gateway/00-rest-client.md), [03-app-removal-and-helpers/03-new-rest-methods-and-free-functions.md](../03-app-removal-and-helpers/03-new-rest-methods-and-free-functions.md); D9 |
...
the typed-decode pipeline (D12); the interactions half is now ALSO resolved (D10-interactions,
maintainer): interactions are app-less — the 9 action helpers are deleted (callers use `rest.*`
directly) and the 8 builder factories the dossier verified as app-free (§7.4) are kept as app-free
sync constructors, so the REST-bot return-a-builder flow is unchanged. Both halves of the original
D10 are resolved; no FLAGGED decision remains.
...
`interaction.create_initial_response` / `user.send` — the exact patterns every example uses. The dossier's suggested
replacement path was `event.app.rest.*` — **since superseded by D10-events**: events are app-less,
the delegating properties (`message_events.py:87-89`) are deleted rather than re-sourced, and the
blessed handler path is closing over the bot object (`bot.rest`, dossier 19 §3.3). The interactions
half is superseded the same way (**D10-interactions RESOLVED**): the 9 interaction action helpers
are deleted and callers hit `rest.*` directly — consistent with the dossier's finding that every
removed helper wraps an existing endpoint, so no new REST method is needed.
...
| D10-interactions — interactions app-less: 9 action helpers deleted, 8 builder factories kept app-free (RESOLVED by maintainer) | 08, 10 |
...
The consolidated, actionable form of the open items (probes, sub-decisions, and tasks — no FLAGGED
decisions remain) is
