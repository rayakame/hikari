...
| 10 | `10-rest-client-surface.md` | REST public methods, deserialize/serialize call sites, DX shift | ~188 async endpoints; `impl/rest.py` has **zero** `.app` refs (REST is structurally safe); 90 helpers wrap **existing** endpoints (no mandatory new endpoint); only `User.send` is genuinely compound; `event.app.rest.*` was the dossier's replacement-path suggestion — since superseded by D10-events (events are app-less; handlers close over the bot object) — and some events derive app from their entity and must switch to storing it | [09-rest-and-gateway/00-rest-client.md](../09-rest-and-gateway/00-rest-client.md), [03-app-removal-and-helpers/03-new-rest-methods-and-free-functions.md](../03-app-removal-and-helpers/03-new-rest-methods-and-free-functions.md); D9 |
...
blessed handler path is closing over the bot object (`bot.rest`, dossier 19 §3.3).
